...
public final class DashboardPage extends BaseTest {
  // Selectores de navegación - usando los XPATHs exactos de ui_elements.json
  private static final By botonEditarPerfil = By.xpath("//a[normalize-space()='Editar Perfil'] | //button[normalize-space()='Editar Perfil']");
  ...
  // Selectores de búsqueda - usando los XPATHs exactos de ui_elements.json
  ...
  /**
   * Verifica si el botón Editar Perfil está visible
   * @return true si está visible, false en caso contrario
   */
  public static boolean isBotonEditarPerfilVisible() {
    try {
      boolean visible = Utils.getElement(botonEditarPerfil).isDisplayed();
      Utils.anotate(Utils.LogLevel.INFO, "Botón Editar Perfil visible: " + visible);
      return visible;
    } catch (Exception e) {
      Utils.anotate(Utils.LogLevel.INFO, "Botón Editar Perfil no visible");
      return false;
    }
  }

  /**
   * Verifica si el botón Configuración está visible
   * @return true si está visible, false en caso contrario
   */
  public static boolean isBotonConfiguracionVisible() {
    try {
      boolean visible = Utils.getElement(botonConfiguracion).isDisplayed();
      Utils.anotate(Utils.LogLevel.INFO, "Botón Configuración visible: " + visible);
      return visible;
    }
  } catch (Exception e) {
      Utils.anotate(Utils.LogLevel.INFO, "Botón Configuración no visible");
      return false;
    }
  }

  /**
   * Verifica si el botón Logout está visible
   * @return true si está visible, false en caso contrario
   */
  public static boolean isBotonLogoutVisible() {
    try {
      boolean visible = Utils.getElement(botonLogout).isDisplayed();
      Utils.anotate(Utils.LogLevel.INFO, "Botón Logout visible: " + visible);
      return visible;
    } catch (Exception e) {
      Utils.anotate(Utils.LogLevel.INFO, "Botón Logout no visible");
      return false;
    }
  }
   ...
   * @return true si está cargado, false en caso contrario
   */
  public static boolean isDashboardLoaded() {
    try {
      String currentUrl = BaseTest.getDriver().getCurrentUrl();
      boolean loaded = currentUrl.contains("/dashboard") && 
                      isBotonEditarPerfilVisible() && 
                      isBotonConfiguracionVisible() && 
                      isBotonLogoutVisible();
      Utils.anotate(Utils.LogLevel.INFO, "Dashboard cargado: " + loaded);
      return loaded;
    } catch (Exception e) {
      Utils.anotate(Utils.LogLevel.WARNING, "Error al verificar carga del dashboard");
      return false;
    }
  }
}